...
def random_location(turtle_index):
  ...
  apple_letters = []
  ind = rand.randint(0,len(letters))
...
def write_letter(letter,turtle_index):
  x = act_apple.xcor()
  y = act_apple.ycor()
  wn.tracer(False)
  turtles[turtle_index].penup()
  ...
  turtles[turtle_index].write(letter,font=("Arial",25,"bold"))
  turtles[turtle_index].penup()
...
def draw_apple(letter,turtle_index):
...
def apple_drop(letter_index):
  ...
  turtles[letter_index].goto(xcor,-175)
  random_location(turtles[letter_index])
